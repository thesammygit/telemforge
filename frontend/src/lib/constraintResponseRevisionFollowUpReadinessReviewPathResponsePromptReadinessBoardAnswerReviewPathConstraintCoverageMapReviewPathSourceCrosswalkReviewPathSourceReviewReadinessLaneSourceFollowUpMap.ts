...
function buildStaticCitationCheckPromptCard(
  ...
  return {
    ...
    citationCheckPromptText:
      `Static citation-check prompt card ${sourceStaticSourceFollowUpCueCardId}: use Stage 104 static source-follow-up cue card ${sourceStaticSourceFollowUpCueCardId}, matched Stage 105 source follow-up map entries ${joinOrNone(sourceSourceFollowUpMapEntryIds)}, Stage 104 source-readiness lane rows ${joinOrNone(staticSourceFollowUpCueCard.sourceSourceReviewReadinessLaneRowIds)}, Stage 103 static source-review prompt card ${staticSourceFollowUpCueCard.sourceStaticSourceReviewPromptCardId}, Stage 102 static review-check card ${staticSourceFollowUpCueCard.sourceStaticReviewCheckCardId}, Stage 101 static response-review prompt cards ${joinOrNone(staticSourceFollowUpCueCard.sourceStaticResponseReviewPromptCardIds)}, Stage 100 static response-note prompt card ${staticSourceFollowUpCueCard.sourceStaticResponseNotePromptCardId}, Stage 98 readiness row ${staticSourceFollowUpCueCard.sourceResponsePromptReadinessRowId}, Stage 97 response-prompt card ${joinOrNone(staticSourceFollowUpCueCard.sourceStaticResponsePromptCardIds)}, Stage 96 response-map rows ${joinOrNone(staticSourceFollowUpCueCard.sourceResponseMapRowIds)}, anchors ${joinOrNone(staticSourceFollowUpCueCard.sourceLocalAnchorHrefs)}, callbacks ${joinOrNone(staticSourceFollowUpCueCard.evidenceCallbackIds)}, gap prompts ${joinOrNone(staticSourceFollowUpCueCard.gapDiscussionPointIds)}, deferred reminders ${joinOrNone(staticSourceFollowUpCueCard.deferredScopeReminderIds)}, citation-check labels ${joinOrNone(staticCitationCheckLabels)}, source-follow-up cue labels ${joinOrNone(staticSourceFollowUpCueCard.staticSourceFollowUpCueLabels)}, and Stage 104 static source-follow-up cue text "${displayCarriedText(staticSourceFollowUpCueCard.staticSourceFollowUpCueText)}" as deterministic manual citation-check context only.`,
    staticNonGoalContext:
