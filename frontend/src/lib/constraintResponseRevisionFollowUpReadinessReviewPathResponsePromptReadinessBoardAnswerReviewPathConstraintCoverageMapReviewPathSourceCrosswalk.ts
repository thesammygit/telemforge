...
function buildStaticReviewCheckCard(
  ...
  return {
    ...
    staticReviewCheckText:
      `Static review-check card ${sourceCrosswalkStaticReviewCheckCardId}: verify Stage 101 response-prompt card ${sourceStaticResponsePromptCardId}, Stage 100 response-note prompt card ${staticResponsePromptCard.sourceStaticResponseNotePromptCardId}, matched Stage 101 review-path steps ${joinOrNone(sourceConstraintCoverageReviewPathStepIds)}, Stage 100 constraint-coverage row ${staticResponsePromptCard.sourceConstraintCoverageRowId}, Stage 99 answer-review step ${staticResponsePromptCard.sourceAnswerReviewPathStepId}, Stage 98 readiness row ${staticResponsePromptCard.sourceResponsePromptReadinessRowId}, Stage 97 response-prompt card ${staticResponsePromptCard.sourceStaticResponsePromptCardId}, anchors ${joinOrNone(staticResponsePromptCard.sourceLocalAnchorHrefs)}, callbacks ${joinOrNone(staticResponsePromptCard.evidenceCallbackIds)}, gap prompts ${joinOrNone(staticResponsePromptCard.gapDiscussionPointIds)}, deferred reminders ${joinOrNone(staticResponsePromptCard.deferredScopeReminderIds)}, review-check labels ${joinOrNone(staticReviewCheckLabels)}, source-crosswalk labels ${joinOrNone(buildSourceCrosswalkLabelsForCard(staticResponsePromptCard, matchedSourceCrosswalkRows))}, and carried static response-prompt text "${staticResponsePromptCard.staticResponsePromptText}" as static manual source-check context only.`,
    staticNonGoalContext:
