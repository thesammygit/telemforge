...
test("buildConstraintResponseRevisionFollowUpReadinessReviewPathResponsePromptReadinessBoardAnswerReviewPathConstraintCoverageMapReviewPathSourceCrosswalk derives Stage 102 source crosswalk from Stage 101", async () => {
  const {
    ...
    assert.fail(`expected Stage 102 helper module to exist: ${error.message}`),
  );
  ...
  assert.strictEqual(
    sourceCrosswalk.sourceConstraintResponseRevisionFollowUpReadinessReviewPathResponsePromptReadinessBoardAnswerReviewPathConstraintCoverageMapReviewPath,
    sourceReviewPath,
  );
  assert.equal(
  ...
  assert.deepEqual(
    ...
    sourceReviewPath.staticResponsePromptCards.map((card) => [
      ...
      sourceCrosswalk.sourceCrosswalkRows
        .filter((row) =>
          row.sourceStaticResponsePromptCardIds.includes(card.staticResponsePromptCardId),
        )
      ...
      sourceCrosswalk.staticReviewCheckCards.find(
        (checkCard) =>
          checkCard.sourceStaticResponsePromptCardId === card.staticResponsePromptCardId,
  ...
  assert.ok(
    sourceCrosswalk.sourceCrosswalkRows.every(
      (row) =>
        row.constraintResponseRevisionFollowUpReadinessReviewPathResponsePromptReadinessBoardAnswerReviewPathConstraintCoverageMapReviewPathSourceCrosswalkRowId.length > 0 &&
        ...
        row.sourceCrosswalkText.includes(row.sourceConstraintCoverageReviewPathStepId) &&
        row.sourceCrosswalkText.includes(row.sourceStaticResponsePromptCardIds[0]) &&
        row.staticReviewCheckText.includes(row.sourceConstraintCoverageReviewPathStepId) &&
  ...
  assert.ok(
    sourceCrosswalk.staticReviewCheckCards.every(
      (card) =>
        card.constraintResponseRevisionFollowUpReadinessReviewPathResponsePromptReadinessBoardAnswerReviewPathConstraintCoverageMapReviewPathSourceCrosswalkStaticReviewCheckCardId.length > 0 &&
