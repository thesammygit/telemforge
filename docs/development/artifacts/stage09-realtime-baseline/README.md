...
- `stage09-baseline-review-packet.json`: deterministic public review packet
  that ties the acceptance matrix, closeout gate, command evidence, stable
...
- `stage09-baseline-handoff-summary.md`: deterministic human-readable summary
  generated from the handoff gate. It lists the review verdict, stable
  fingerprint, passed/missed metrics, runtime promotion blocker, required next
  evidence, local resource envelope, and Rust data-plane-only scope without
...
python3 scripts/summarize_stage09_baseline_closeout.py
...
python3 scripts/summarize_stage09_baseline_handoff.py
...
python3 -m unittest tests/contracts/test_stage09_baseline_handoff_summary.py
