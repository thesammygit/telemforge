# Stage 49 Local Review Observation Handoff Relay Trail Slice

## Contract

Stage 49 adds a deterministic local
`telemforge.review_observation_handoff_relay_trail.v1` surface over the Stage 48
`reviewObservationHandoffSynthesis` output. Relay trail steps preserve Stage 48
synthesis row order, and static inspection notes preserve Stage 48 static relay
note order.

The surface is static manual-review context only. It is not saved reviewer
notes, saved relay progress, saved inspection state, saved synthesis state,
...
certification, meeting workflow, handoff package generation, runnable checklist
behavior, task launchers, or command execution.

## Source Files

- `frontend/src/lib/reviewObservationHandoffRelayTrail.ts`
- `frontend/src/features/mission-console/types.ts`
- `frontend/src/features/mission-console/consoleViewModel.ts`
- `frontend/src/features/mission-console/MissionConsole.tsx`
- `frontend/src/styles/global.css`
- `tests/frontend/reviewObservationHandoffRelayTrail.test.ts`
- `tests/frontend/consoleViewModel.test.ts`

## Human Test Gate

Open the mission console in fixture mode, find the Stage 49 relay trail near the
Stage 48 synthesis map, confirm relay step order mirrors Stage 48 synthesis row
order, confirm static inspection notes mirror Stage 48 relay note order, follow
...
synthesis state, saved calibration state, saved drift state, reviewer identity,
