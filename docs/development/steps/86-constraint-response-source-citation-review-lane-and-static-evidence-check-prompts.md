# Stage 86: Constraint Response Source Citation Review Lane And Static Evidence Check Prompts

## Goal

Turn the completed Stage 85 source follow-up map and static citation-check
prompt cards into a deterministic local source citation-review lane and static
evidence-check prompt surface so reviewers can inspect citation prompts,
follow-up map entries, source lineage, local anchors, and evidence callbacks
...
citation-review state, saved evidence-check state, owner assignment, ticketing,
...
- derives ordered citation-review lane rows from Stage 85 static
  citation-check prompt cards;
- derives static evidence-check prompt cards from Stage 85 source follow-up map
  entries;
- preserves Stage 85 citation prompt order and source follow-up map entry
  order;
- carries the Stage 85 default follow-up context into the Stage 86 summary;
- exposes Stage 85 citation prompt card ids, Stage 85 follow-up map entry ids,
  Stage 84 readiness row ids, Stage 84 cue card ids, Stage 83 through Stage 64
  source ids, local anchors, callbacks, gap prompts, deferred reminders,
  labels, citation-review text, evidence-check prompt text, local-only flags,
...
Recommended: start with Option A. Stage 86 should make Stage 85 citation-check
cards and follow-up map entries easier to inspect as a citation-review lane
...
Option A: compact citation-review lane panel near the Stage 85 panel

- keeps citation-review context adjacent to the Stage 85 source follow-up map
...
Recommended: Option A. The first source citation-review lane should be a
compact read-only mission-console panel.

## Work Items

- add a deterministic local helper,
  `frontend/src/lib/constraintResponseSourceCitationReviewLane.ts`, over the
  Stage 85 source follow-up map view;
- define compact Stage 86 types in
...
- wire the citation-review lane into
  ...
  85 source follow-up map is built, without changing fixture or local-live
  boundaries;
- surface a compact Stage 86 citation-review lane/static evidence-check panel
  in `frontend/src/features/mission-console/MissionConsole.tsx` near the Stage
  85 panel;
- update `frontend/src/styles/global.css` only as needed for the compact Stage
  86 panel;
- add focused frontend tests in
  `tests/frontend/constraintResponseSourceCitationReviewLane.test.ts` and
  update `tests/frontend/consoleViewModel.test.ts` where needed;
- add a public-safe Stage 86 artifact under
  `docs/development/artifacts/stage86-constraint-response-source-citation-review-lane/`
...
2. find the Stage 86 source citation-review lane near the Stage 85 source
   follow-up map;
3. confirm citation-review lane row order preserves Stage 85 static
   citation-check prompt card order;
4. confirm static evidence-check prompt card order preserves Stage 85 source
   follow-up map entry order;
5. confirm the default citation-review context mirrors the Stage 85 default
   follow-up context;
6. confirm each citation-review lane row shows Stage 85 citation prompt ids,
   Stage 85 source follow-up map entry ids, Stage 84 readiness row ids, Stage
   84 cue card ids, Stage 83 through Stage 64 source ids, local anchors,
   callbacks, gap prompts, deferred reminders, labels, citation-review text,
   evidence-check prompt text, and static non-goal context;
...
- focused frontend model tests proving citation-review lane rows derive from
  Stage 85 static citation-check prompt cards and static evidence-check prompt
  cards derive from Stage 85 source follow-up map entries;
- assertions that citation-review lane row order, static evidence-check prompt
  ...
  remain stable;
- assertions that each citation-review row carries Stage 85 through Stage 64
...
- existing Stage 85 through Stage 09 checks as regression coverage for touched
  surfaces;
...
- citation-review lane rows derive from Stage 85 static citation-check prompt
  cards and static evidence-check prompt cards derive from Stage 85 source
...
- Stage 85 citation prompt cards and follow-up map entries, Stage 84 readiness
  row ids and cue card ids, Stage 83 source-review path steps and static
  source-review prompt cards, Stage 82 source-crosswalk rows and static
  review-check cards, Stage 81 review-path steps and static response-review
  prompt cards, Stage 80 constraint-coverage rows and static response-note
  prompt cards, Stage 79 answer-review path steps and static constraint-note
  cards, Stage 78 static answer-check cards and readiness rows, Stage 77
  ...
  gaps, deferred reminders, labels, citation-review text, and static
  evidence-check prompt text are explicit and source-backed;
...
- the mission-console panel is compact and adjacent to Stage 85 without route
