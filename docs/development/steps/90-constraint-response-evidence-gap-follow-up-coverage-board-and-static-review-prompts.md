# Stage 90: Constraint Response Evidence Gap Follow-Up Coverage Board And Static Review Prompts

## Goal

Turn the completed Stage 89 evidence-gap follow-up review path and static
readiness cues into a deterministic local follow-up coverage board and static
review prompt surface so reviewers can see which review steps, readiness cues,
source lineage, anchors, callbacks, gap prompts, and deferred reminders are
covered before drafting outside the app.
...
saved evidence-gap follow-up selections, saved coverage-board selections,
...
Option A: deterministic local follow-up coverage board and static review
prompts

- derives ordered coverage rows from Stage 89 follow-up review path steps;
- derives static review prompt cards from Stage 89 static readiness cue cards;
- preserves Stage 89 follow-up review step order and static readiness cue card
  order;
- carries the Stage 89 default follow-up review context into the Stage 90
  summary;
- exposes Stage 89 follow-up review path step ids, Stage 89 static readiness
  cue card ids, Stage 88 readiness row ids, Stage 88 static follow-up prompt
  card ids, Stage 87 through Stage 64 source ids, local anchors, callbacks, gap
  prompts, deferred reminders, coverage labels, prompt text, local-only flags,
...
Recommended: start with Option A. Stage 90 should make Stage 89 review steps
and readiness cue cards easier to scan as coverage context without adding saved
state, workflow, scoring, certification, exports, commands, routing,
ownership, or production handoff semantics.

### Placement

Option A: compact follow-up coverage board near the Stage 89 panel

- keeps coverage context adjacent to the Stage 89 follow-up review path it
  derives from;
- lets reviewers compare review steps, readiness cue cards, source lineage,
  anchors, callbacks, gap prompts, deferred reminders, and static review
...
Recommended: Option A. The first evidence-gap follow-up coverage board should
...
- add a deterministic local helper,
  ...
  over the Stage 89 evidence-gap follow-up review path view;
- define compact Stage 90 types in
...
- wire the coverage board into
  ...
  89 evidence-gap follow-up review path is built, without changing fixture or
  local-live boundaries;
- surface a compact Stage 90 evidence-gap follow-up coverage board/static
  review prompt panel in
  `frontend/src/features/mission-console/MissionConsole.tsx` near the Stage 89
  panel;
...
- add a public-safe Stage 90 artifact under
  `docs/development/artifacts/stage90-constraint-response-evidence-gap-follow-up-coverage-board/`
...
2. find the Stage 90 evidence-gap follow-up coverage board near the Stage 89
   evidence-gap follow-up review path;
3. confirm coverage row order preserves Stage 89 follow-up review path step
   order;
4. confirm static review prompt card order preserves Stage 89 static readiness
   cue card order;
5. confirm the default Stage 90 coverage context mirrors the Stage 89 default
   follow-up review context;
6. confirm each coverage row shows Stage 89 follow-up review path step ids,
   Stage 89 static readiness cue card ids, Stage 88 readiness row ids, Stage
   88 static follow-up prompt card ids, Stage 87 through Stage 64 source ids,
...
8. confirm the panel is static manual-review context only and does not become
   ...
   evidence-gap follow-up selections, coverage-board selections, saved coverage
...
- no saved reviewer answers, saved answer drafts, saved reviewer notes, saved
  ...
  evidence-gap follow-up selections, saved coverage-board selections, saved
  coverage state, local storage, persistence, saved review sessions, saved
...
- focused frontend model tests proving coverage rows derive from Stage 89
...
- assertions that each coverage row carries Stage 89 through Stage 64 source
...
- existing Stage 89 through Stage 09 checks as regression coverage for touched
  surfaces;
...
- saved reviewer answers, saved answer drafts, saved reviewer notes, saved
  response notes, saved source selections, saved citation selections, saved
  evidence-gap follow-up selections, saved coverage-board selections, saved
  coverage state, saved review progress, local storage, or persistence tests;
...
- coverage rows derive from Stage 89 follow-up review path steps and static
  review prompt cards derive from Stage 89 static readiness cue cards, not ad
  hoc UI strings;
- coverage row order, static review prompt card order, default context, labels,
  and source/anchor reference order remain stable;
- Stage 89 follow-up review path steps and static readiness cue cards, Stage 88
  evidence-gap readiness rows and static follow-up prompt cards, Stage 87
  evidence-check review path steps and static citation-gap cue cards, Stage 86
  through Stage 64 source ids, local anchors, callbacks, gaps, deferred
...
- the mission-console panel is compact and adjacent to Stage 89 without route
