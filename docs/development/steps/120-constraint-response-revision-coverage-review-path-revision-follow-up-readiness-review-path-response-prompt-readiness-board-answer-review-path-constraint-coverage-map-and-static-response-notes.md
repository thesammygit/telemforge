# Stage 120: Constraint Response Revision Coverage Review Path Revision Follow-Up Readiness Review Path Response Prompt Readiness Board Answer Review Path Constraint Coverage Map And Static Response Notes

## Goal

Turn the completed Stage 119 answer-review path and static constraint-note
surface into a deterministic local constraint-coverage map and static
response-note prompt surface. A reviewer should be able to verify which manual
answer constraints support each answer-review step before drafting the next
...
- derives constraint-coverage rows from Stage 119 answer-review path steps;
- derives static response-note prompts from Stage 119 static constraint-note
  cards;
- preserves Stage 119 answer-review step order and Stage 119 constraint-note
  order;
- carries the Stage 119 default answer-review context into the Stage 120
  summary;
- exposes Stage 119 answer-review step ids, Stage 119 constraint-note card
  ids, Stage 118 answer-check card ids, Stage 118 readiness-row ids, Stage 117
  response-prompt card ids and revision follow-up readiness review-path step
  ids, Stage 116 readiness rows and response-check cards, Stage 115 through
  Stage 64 source lineage ids, anchors, callbacks, gap prompts, deferred
  reminders, answer-review labels, constraint-note labels,
  constraint-coverage labels, and static response-note prompts as manual review
  context only.
...
Recommended: start with Option A. Stage 120 should make Stage 119 constraints
...
Option A: compact constraint-coverage map near the Stage 119 panel
...
Recommended: Option A. The Stage 120 constraint-coverage map should be a
compact read-only mission-console panel near Stage 119.
...
- add a deterministic local helper,
  ...
  over the completed Stage 119 answer-review path view;
- define compact Stage 120 types in
...
- wire the constraint-coverage map into
  ...
  119 answer-review path is built, without changing fixture or local-live
  boundaries;
- surface a compact Stage 120 constraint-coverage map/static response-note
  prompt panel in `frontend/src/features/mission-console/MissionConsole.tsx`
  near the Stage 119 panel;
...
- keep Stage 119 and Stage 118 helper/test coverage in the verification set as
  regression coverage for touched surfaces;
- add a public-safe Stage 120 artifact under
  `docs/development/artifacts/stage120-constraint-response-revision-coverage-review-path-revision-follow-up-readiness-review-path-response-prompt-readiness-board-answer-review-path-constraint-coverage-map/`
...
2. find the Stage 120 constraint-coverage map near the Stage 119 answer-review
   path;
3. confirm constraint-coverage row order preserves Stage 119 answer-review
   step order;
4. confirm static response-note prompt order preserves Stage 119 static
   constraint-note order;
5. confirm the default response-note context mirrors the Stage 119 default
   answer-review context;
6. confirm each constraint-coverage row shows Stage 119 answer-review step ids,
   Stage 119 constraint-note ids, Stage 118 answer-check and readiness ids,
   Stage 117 response-prompt and review-path ids, Stage 116
   readiness/response-check ids, Stage 115 through Stage 64 source ids, local
   anchors, callbacks, gap prompts, deferred reminders, answer-review labels,
   constraint-note labels, and static response-note prompts;
...
8. confirm the panel is static manual-review context only and does not become
   ...
   saved response-note state, route changes, exports, signoff, audit
...
- focused frontend model tests proving constraint-coverage rows derive from
  Stage 119 answer-review path steps and static response-note prompts derive
  from Stage 119 static constraint-note cards;
...
- assertions that each constraint-coverage row carries Stage 119 through Stage
...
- existing Stage 119 through Stage 09 checks as regression coverage for
  touched surfaces;
...
- constraint-coverage rows derive from Stage 119 answer-review path steps and
  static response-note prompts derive from Stage 119 static constraint-note
...
- Stage 119 answer-review path steps and static constraint-note cards, Stage
  118 static answer-check cards and readiness rows, Stage 117 response-prompt
  cards and review-path steps, Stage 116 readiness rows and response-check
  cards, Stage 115 through Stage 64 source ids, local anchors, callbacks, gaps,
...
- the mission-console panel is compact and adjacent to Stage 119 without route
