# Stage 97: Constraint Response Revision Follow-Up Readiness Review Path And Static Response Prompts

## Goal

Turn the completed Stage 96 revision follow-up readiness board and static
response-check surface into a deterministic local revision follow-up readiness
review path and static response-prompt surface so reviewers can walk from each
readiness row to the next manual response prompt before editing outside the app.
...
revision follow-up readiness selections, saved response-check selections, saved
response-prompt selections, saved review-path state, owner assignment,
...
- derives review-path steps from Stage 96 revision follow-up readiness rows;
- derives static response-prompt cards from Stage 96 static response-check
  cards;
- preserves Stage 96 readiness row order and static response-check card order;
- carries the Stage 96 default revision follow-up readiness context into the
  Stage 97 summary;
- exposes Stage 96 readiness row ids, Stage 96 static response-check card ids,
  Stage 95 review-path step ids, Stage 95 static follow-up prompt card ids,
  Stage 94 through Stage 64 source ids, local anchors, callbacks, gap prompts,
  deferred reminders, deterministic review-path labels, static response prompt
...
Recommended: start with Option A. Stage 97 should make Stage 96 readiness
...
Option A: compact revision follow-up readiness review path near the Stage 96
panel
...
Recommended: Option A. The first revision follow-up readiness review path
...
- add a deterministic local helper,
  ...
  over the Stage 96 revision follow-up readiness board view;
- define compact Stage 97 types in
...
- wire the review path into
  ...
  96 revision follow-up readiness board is built, without changing fixture or
  local-live boundaries;
- surface a compact Stage 97 revision follow-up readiness review path/static
  response-prompt panel in
  `frontend/src/features/mission-console/MissionConsole.tsx` near the Stage 96
  panel;
...
- add a public-safe Stage 97 artifact under
  `docs/development/artifacts/stage97-constraint-response-revision-follow-up-readiness-review-path/`
...
2. find the Stage 97 revision follow-up readiness review path near the Stage 96
   readiness board;
3. confirm review-path step order preserves Stage 96 readiness row order;
4. confirm static response-prompt card order preserves Stage 96 static
   response-check card order;
5. confirm the default Stage 97 context mirrors the Stage 96 default revision
   follow-up readiness context;
6. confirm each review-path step shows Stage 96 readiness row ids, Stage 96
   static response-check card ids, Stage 95 review-path step ids, Stage 95
   static follow-up prompt card ids, Stage 94 through Stage 64 source ids,
   local anchors, callbacks, gap prompts, deferred reminders, review-path
...
- focused frontend model tests proving review-path steps derive from Stage 96
  readiness rows and static response-prompt cards derive from Stage 96 static
...
- assertions that each review-path step carries Stage 96 through Stage 64
...
- existing Stage 96 through Stage 09 checks as regression coverage for touched
  surfaces;
...
- one deterministic local constraint-response revision follow-up readiness
  review path and static response-prompt surface is source-backed and
  visible/testable;
- review-path steps derive from Stage 96 revision follow-up readiness rows and
  static response-prompt cards derive from Stage 96 static response-check cards,
...
- Stage 96 readiness rows and response-check cards, Stage 95 review-path steps
  and static follow-up prompt cards, Stage 94 through Stage 64 source ids, local
  anchors, callbacks, gaps, deferred reminders, labels, and response prompt
...
- the mission-console panel is compact and adjacent to Stage 96 without route
