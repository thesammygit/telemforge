...
- derives ordered citation-review lane rows from Stage 125 static
  citation-check prompt cards;
- derives static evidence-check prompt cards from Stage 125 source follow-up
  map entries;
- preserves Stage 125 citation prompt order and source follow-up map entry
  order;
- carries the Stage 125 default follow-up context into the Stage 126 summary;
- exposes Stage 125 citation prompt card ids, Stage 125 follow-up map entry
  ids, Stage 124 readiness row ids, Stage 124 cue card ids, Stage 123 source
  review path step ids, Stage 123 static source-review prompt ids, Stage 122
  source-crosswalk row ids, Stage 122 static review-check card ids, Stage 121
  through Stage 64 source lineage ids, local anchors, callbacks, gap prompts,
  deferred reminders, labels, citation-review text, evidence-check prompt
...
Recommended: start with Option A. Stage 126 should make Stage 125
...
Option A: compact citation-review lane panel near the Stage 125 panel

- keeps citation-review context adjacent to the Stage 125 source follow-up map
...
Recommended: Option A. The Stage 126 source citation-review lane should be a
...
- add a deterministic local helper,
  ...
  over the Stage 125 source follow-up map view;
- define compact Stage 126 types in
...
- wire the citation-review lane into
  ...
  125 source follow-up map is built, without changing fixture or local-live
  boundaries;
- surface a compact Stage 126 citation-review lane/static evidence-check panel
...
- keep Stage 125 through Stage 116 helper/test coverage in the verification set
  as regression coverage for touched surfaces;
- add a public-safe Stage 126 artifact under
  `docs/development/artifacts/stage126-constraint-response-source-citation-review-lane/`
...
2. find the Stage 126 source citation-review lane near the Stage 125 source
   follow-up map;
3. confirm citation-review lane row order preserves Stage 125 static
   citation-check prompt card order;
4. confirm static evidence-check prompt card order preserves Stage 125 source
   follow-up map entry order;
5. confirm the default citation-review context mirrors the Stage 125 default
   follow-up context;
6. confirm each citation-review lane row shows Stage 125 citation prompt ids,
   Stage 125 source follow-up map entry ids, Stage 124 readiness row ids,
   Stage 124 cue card ids, Stage 123 source-review path steps and prompts,
   Stage 122 source-crosswalk rows and checks, Stage 121 through Stage 64
   source ids, local anchors, callbacks, gap prompts, deferred reminders,
   labels, citation-review text, evidence-check prompt text, and static
   non-goal context;
...
- focused frontend model tests proving citation-review lane rows derive from
  Stage 125 static citation-check prompt cards and static evidence-check
  prompt cards derive from Stage 125 source follow-up map entries;
- assertions that citation-review lane row order, static evidence-check prompt
  ...
  remain stable;
- assertions that each citation-review row carries Stage 125 through Stage 64
...
- existing Stage 125 through Stage 09 checks as regression coverage for touched
  surfaces;
...
- citation-review lane rows derive from Stage 125 static citation-check prompt
  cards and static evidence-check prompt cards derive from Stage 125 source
...
- Stage 125 citation prompt cards and follow-up map entries, Stage 124
  readiness row ids and cue card ids, Stage 123 source-review path steps and
  static source-review prompt cards, Stage 122 source-crosswalk rows and
  static review-check cards, Stage 121 review-path steps and static
  ...
  citation-review text, and static evidence-check prompt text are explicit and
  source-backed;
...
- the mission-console panel is compact and adjacent to Stage 125 without route
